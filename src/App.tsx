...
import { useTripAudio } from './hooks/useTripAudio'
...
function TravelMotionApp() {
  ...
  const animation = useTripAnimation(trip.legs, trip.speed, trip.story)
  const recorder = useVideoRecorder()
  const music = useTripAudio(trip.story.musicAssetId, trip.story.musicVolume)
  const pwa = usePwa()
  ...
  useEffect(() => {
    if (animation.status === 'completed') {
      music.pause()
      if (recorder.isRecording) void recorder.finish(true)
    }
  }, [animation.status, music.pause, recorder.isRecording, recorder.finish])
  ...
  const handlePlay = () => { void music.playAt(animation.time); animation.play() }
  const handlePause = () => { music.pause(); animation.pause() }
  const handleReplay = () => { void music.playAt(0); animation.replay() }
  const handleSeek = (time: number) => { music.seek(time); animation.seek(time) }

  const startExport = async () => {
    if (!canvasRef.current || trip.locations.length < 2) return
    animation.seek(0)
    const audioStream = await music.recordingStream()
    await music.playAt(0)
    requestAnimationFrame(() => {
      if (canvasRef.current && recorder.start(canvasRef.current, trip.name, trip.language, audioStream)) animation.startRecording()
      else music.pause()
    })
  }
  ...
  return (
    <main className="relative h-[100dvh] min-h-0 overflow-hidden bg-[var(--app-bg)] text-[var(--text)] lg:grid lg:grid-cols-[384px_minmax(0,1fr)]">
