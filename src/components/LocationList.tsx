...
interface Props {
  ...
  onLocationChange: (index: number, patch: Partial<Location>) => void
}
...
export function LocationList({ locations, legs, language, speed, disabled, onRemove, onMove, onReorder, onLegChange, onLocationChange }: Props) {
  ...
  return <div>{locations.map((location, index) => {
    ...
    return <div key={location.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => drop(event, index)}>
      <div draggable={!disabled} onDragStart={(event) => startDrag(event, index)} onDragEnd={() => setDraggedIndex(null)} className={`group grid min-h-[64px] grid-cols-[24px_32px_minmax(0,1fr)_auto] items-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-2 py-2 transition-opacity ${draggedIndex === index ? 'opacity-45' : ''}`}>
        ...
        <div className="flex items-center gap-0.5">
          <button type="button" disabled={disabled} onClick={() => setExpandedStory(expandedStory === index ? null : index)} className={`icon-button !h-8 !w-8 ${expandedStory === index ? 'bg-[var(--surface-raised)] !text-[var(--warm)]' : ''}`} aria-expanded={expandedStory === index} aria-label={`${t(language, 'locationStory')} ${name}`}><Icon name="story" size={15} /></button>
      ...
      {expandedStory === index && <LocationStoryPanel location={location} language={language} disabled={disabled} onChange={(patch) => onLocationChange(index, patch)} />}
      {index < locations.length - 1 && <div className="relative ml-4 border-l border-[var(--border-strong)] py-1.5 pl-3">
        ...
        {expandedLeg === index && <div className="mb-2 mt-1 grid grid-cols-2 gap-2 rounded-lg border border-[var(--border)] bg-[var(--app-bg)] p-3">
          ...
          <label className="col-span-2"><span className="mb-1.5 flex justify-between text-[11px] font-medium text-[var(--text-muted)]"><span>{t(language, 'cameraZoom')}</span><span className="tabular-nums">{(leg?.cameraZoom ?? 1.78).toFixed(2)}×</span></span><input type="range" min={1.2} max={2.3} step={.05} value={leg?.cameraZoom ?? 1.78} disabled={disabled} onChange={(event) => onLegChange(index, { cameraZoom: Number(event.target.value) })} className="timeline-range" /></label>
        </div>}
      </div>}
    </div>
  })}</div>
...
function LocationStoryPanel({ location, language, disabled, onChange }: { location: Location; language: Language; disabled?: boolean; onChange: (patch: Partial<Location>) => void }) {
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState(false)
  const choosePhoto = async (file: File) => {
    setSaving(true); setError(false)
    try {
      const asset = await saveMediaFile(file, 'photo')
      await deleteMediaAsset(location.photoAssetId)
      onChange({ photoAssetId: asset.id, photoName: asset.name })
    } catch { setError(true) }
    finally { setSaving(false) }
  }
  const removePhoto = async () => {
    await deleteMediaAsset(location.photoAssetId).catch(() => undefined)
    onChange({ photoAssetId: undefined, photoName: undefined })
  }
  return <div className="mx-1 my-2 space-y-3 rounded-lg border border-[var(--border)] bg-[var(--app-bg)] p-3">
    <label><span className="section-label !mb-1.5">{t(language, 'travelDate')}</span><input type="date" disabled={disabled} value={location.date ?? ''} onChange={(event) => onChange({ date: event.target.value })} className="field-input !min-h-9 !rounded-lg !text-xs" /></label>
    <label><span className="section-label !mb-1.5">{t(language, 'storyNote')}</span><textarea disabled={disabled} value={location.story ?? ''} maxLength={140} rows={3} placeholder={t(language, 'storyPlaceholder')} onChange={(event) => onChange({ story: event.target.value })} className="field-input min-h-[76px] resize-none py-2 leading-5" /></label>
    <div><span className="section-label !mb-1.5">{t(language, 'locationPhoto')}</span>{location.photoName && <p className="mb-2 truncate text-xs text-[var(--text-muted)]">{location.photoName}</p>}<div className="grid grid-cols-2 gap-2"><label className="secondary-button !min-h-9 cursor-pointer !px-2 !text-xs"><Icon name="image" size={14} />{t(language, location.photoAssetId ? 'replacePhoto' : 'choosePhoto')}<input type="file" accept="image/*" disabled={disabled || saving} className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void choosePhoto(file); event.currentTarget.value = '' }} /></label><button type="button" disabled={disabled || !location.photoAssetId} onClick={() => void removePhoto()} className="secondary-button !min-h-9 !px-2 !text-xs"><Icon name="trash" size={14} />{t(language, 'removePhoto')}</button></div></div>
    {error && <p className="text-xs text-[var(--danger)]">{t(language, 'mediaError')}</p>}
  </div>
}
